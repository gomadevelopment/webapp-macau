<div class="row mb-5">
    <div class="col-sm-12 col-md-12 col-lg-12">
        <div class="card-body">
            ...
            <input type="hidden" name="" id="listening_shop_questions_count" value="{{ $listening_shop_questions->count() }}">

            @if($listening_shop_questions->count())

                <div class="custom-tab customize-tab tabs_creative">
                    <ul class="nav nav-tabs p-0 b-0 m-auto" id="perform_listening_shop_tabs" role="tablist">
                        @foreach ($listening_shop_questions as $question)
                            <li class="nav-item">
                            ...
                            <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" data-subtype-id="{{ $question->question_subtype_id }}"
                                ...
                                <div class="d-block text-center mt-4 mb-4">
                                    @if($loop->first)
                                        ...
                                        @else
                                            <a href="#ex{{$listening_shop_questions[$loop->index+1]->id}}" class="btn search-btn comment_submit m-2 perform_exercise_nav_button" style="float: none; padding: 15px 25px;">
                                                <img src="{{asset('/assets/backoffice_assets/icons/play.svg')}}" alt="" style="margin-right: 5px; margin-bottom: 2px;">
                                                Seguinte
                                            </a>
                                        @endif
                                    @else
                                        @if($loop->last)
                                            <a href="#ex{{$listening_shop_questions[$loop->index-1]->id}}" class="btn btn-theme remove_button m-2 perform_exercise_nav_button" style="float: none; padding: 15px 25px;">
                                                <img src="{{asset('/assets/backoffice_assets/icons/small_arrow_back.svg')}}" alt="" style="margin-right: 10px; margin-bottom: 2px;">
                                                Voltar
                                            </a>
                                            <a href="#after-listening" class="btn search-btn comment_submit m-2 perform_exercise_nav_button" style="float: none; padding: 15px 25px;">
